...
describe("release workflow", () => {
  ...
  test("binds the admitted canonical installer without claiming runtime rollout", async () => {
    ...
    // The package README is an immutable release-bound document and remains
    // candidate wording. The current docs and site are updated separately.
    expect(readme).toContain("Local CLI v0.8.1 is a release candidate");
    expect(readme).toContain("The v0.8.1 candidate is not yet admitted");
    expect(releaseNotes).toContain("## Admitted v0.8.1 canonical artifact");
    expect(releaseNotes).toContain("release run 34781400584");
    expect(releaseNotes).toContain("optional exact-byte npm mirror remains a separate provider admission");
    expect(releaseNotes).not.toContain("This release candidate is not yet admitted");
    expect(releaseNotes).toContain("## Admitted v0.7.0 predecessor");
    expect(releaseNotes).toContain("## Admitted v0.7.1 predecessor");
    for (const surface of [availability, gettingStarted]) {
      expect(surface).toContain("v0.8.1");
      expect(surface).toContain("artifact admitted");
      expect(surface).toContain("optional npm mirror requires separate admission");
      expect(surface).toContain("daemon startup");
      expect(surface).toContain(installCommand);
    }
    for (const guide of [availability, gettingStarted]) {
      expect(guide).not.toContain("v0.8.1 is released");
      expect(guide).not.toContain("v0.8.1 CLI passed immutable GitHub and npm artifact admission");
    }
    ...
    expect(availability).toContain("Its optional npm mirror requires separate admission.");
    expect(availability).not.toContain("The v0.8.1 candidate is not yet admitted");
    expect(availability).not.toContain("v0.8.1 is released");
    ...
    expect(readme).toContain("Do not initialize, start, or autostart either the admitted v0.8.0 daemon or the v0.8.1 candidate");
    for (const document of [availability]) {
      expect(document).toContain("Current daemon and hosted command-writer rollout remains blocked on capacity.");
      expect(document).toContain("Do not initialize, start, or autostart either the admitted v0.8.1 daemon or the v0.8.1 release");
    ...
    expect(releaseNotes).toContain("# Oompa v0.8.1 local CLI release\n");
    ...
    expect(thirdPartyNotices).toContain("The `v0.8.1` candidate records its build graph");
    ...
    expect(changelog).toContain("## v0.8.1 candidate (unreleased)\n");
    expect(changelog).toContain("## v0.8.0 candidate (unreleased)\n");
    expect(changelog).not.toContain("## v0.8.1\n");
    ...
    expect(security).toContain("| `v0.8.1` candidate | Not admitted or supported as a public release.");
  ...
  test("binds v0.7.1 admission copy and recovery instructions to their own exact evidence", async () => {
    ...
    expect(releaseRecord).toContain("Status: `v0.8.1` is the admitted canonical GitHub release.");
  ...
  test("keeps the retired fallback-bound path unreachable and exposes only the exact artifact workflow", async () => {
    ...
    expect(releaseRecord.split("\n")[2]).toContain("Status: `v0.8.1` is the admitted canonical GitHub release");
    expect(releaseRecord.split("\n")[2]).toContain("optional npm mirror failed before publication with an OIDC exchange rejection and is not admitted");
    ...
    expect(releaseRecord).toContain("`v0.8.1` is the current admitted canonical GitHub artifact; `v0.7.1` remains an admitted npm predecessor");
    expect(releaseRecord).toContain("The canonical README and website separate canonical GitHub artifact admission from optional npm mirroring and runtime activation.");
    expect(releaseRecord).toContain("The `v0.8.1` canonical GitHub artifact is admitted; its optional npm mirror is not");
    ...
    expect(releaseRecord).toContain("Current source points to the [reviewed v0.8.1 canonical installation notes](beta-release-notes.md#admitted-v081-canonical-artifact)");
