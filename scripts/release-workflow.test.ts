...
describe("release workflow", () => {
  ...
  test("binds the candidate installer without claiming artifact admission or runtime rollout", async () => {
    ...
    expect(readme).not.toContain("Local CLI v0.8.1 is the fully admitted public artifact");
    expect(readme).not.toContain("Install and verify the admitted v0.8.1 CLI artifact");
    expect(readme).toContain("Only after immutable GitHub release admission, install and verify the v0.8.1 candidate CLI artifact. This does not start the daemon:");
    ...
    for (const guide of [availability, gettingStarted]) {
      ...
      expect(guide).not.toContain("The v0.8.1 CLI passed immutable GitHub and npm artifact admission");
      expect(guide).not.toContain("You can install and check v0.8.1 now");
    }
    ...
    expect(homepageAvailabilityText).toContain("The admitted v0.8.0 CLI has its own");
    expect(homepageAvailabilityText).toContain("The v0.8.1 candidate is not yet admitted.");
    ...
    expect(availability).toContain("The v0.8.0 CLI passed immutable GitHub artifact admission.");
    ...
    expect(availability).toContain("The v0.8.1 candidate is not yet admitted");
    ...
    for (const document of [readme, availability]) {
      ...
      expect(document).toContain("Do not initialize, start, or autostart either the admitted v0.8.0 daemon or the v0.8.1 candidate");
    ...
    expect(releaseNotes).toContain("# Oompa v0.8.1 local CLI candidate\n");
    ...
    expect(thirdPartyNotices).not.toContain("The admitted `v0.8.1` release records its build graph");
  ...
  test("binds v0.7.1 admission copy and recovery instructions to their own exact evidence", async () => {
    ...
    expect(releaseRecord).toContain("`v0.8.0` is the current admitted canonical GitHub artifact; `v0.7.1` remains an admitted npm predecessor.");
  ...
  test("keeps the retired fallback-bound path unreachable and exposes only the exact artifact workflow", async () => {
    ...
    expect(releaseRecord.split("\n")[2]).toContain("Status: `v0.8.1` is an unreleased provider-usage foundation candidate");
    expect(releaseRecord.split("\n")[2]).toContain("`v0.8.0` is the admitted canonical GitHub predecessor");
    ...
    expect(releaseRecord).toContain("The `v0.8.0` canonical GitHub artifact is admitted; the npm mirror is not");
    ...
    expect(releaseRecord).toContain("v0.8.1 candidate command, explicitly unavailable until its own exact artifact admission");
    expect(releaseRecord).toContain("links to the [reviewed v0.8.0 canonical installation notes](beta-release-notes.md#admitted-v080-canonical-artifact) for the existing artifact");
