...
describe("installer pins", () => {
  ...
  test("release consistency requires the tagged runtime bytes and matching URLs", async () => {
    ...
    const drift = releasePinDrift(consistent, "v0.8.4", "0.8.4");
    expect(drift).toEqual([]);
    expect(releasePinDrift(report, "v0.8.4", "0.8.4").some((line) => line.includes("public command digest") || line.includes("is not the public command digest") || line.length === 0)).toBe(report.runtime.publicCommand !== report.runtime.actual);
    expect(releasePinDrift(consistent, "v0.1.8", "0.8.4")).toContain("release tag v0.1.8 does not match package.json version 0.8.4");
    for (const priorTag of ["v0.7.0", "v0.7.1", "v0.8.0"]) {
      expect(releasePinDrift(consistent, priorTag, "0.8.4"))
        .toContain(`release tag ${priorTag} does not match package.json version 0.8.4`);
    }
    expect(() => releasePinDrift(consistent, "0.8.4", "0.8.4")).toThrow();
  });
  ...
  test("release preparation alone moves the public runtime digest after inner pins converge", async () => {
    ...
    expect(await updateInstallPinsForRelease(fixture, "v0.8.4", {
      readPins: async () => report,
    }))
      .toEqual(["src/install-preflight.ts: replaced 1 public runtime digest site for v0.8.4"]);
