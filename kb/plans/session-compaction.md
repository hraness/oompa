...
- 2026-09-17: main convergence — the feature was ported off the `claude/oompa-v0.8.0` release line onto updated `main` as `devin/session-compaction-main` via cherry-pick (the 161-file line divergence made a direct merge impractical; cherry-picking confined conflicts to the compaction surface). The migration renumbered v61→v62 because main's own schema 61 is the Devin readmission (#225): the chain is now v60→v61 (retired-provider trigger drop + Devin queue-transcript guard) →v62 (`session_compact_policies`), and a new `initialVersion === 61` step proves the released joined-61 boundary before the additive v62 step. `current60` test names kept (cohort vintage, not current version); every `user_version`/ledger-tail/`MIGRATION_REQUIRED` literal bumped 61→62; future-ledger sentinels moved 62→63; `joined-state-upgrade` rewind test now drops the v62 objects to simulate an authentic v60 root. Devin gained `compact` on `PinnedDevinRuntimeManager` (`/compact` sent as an ACP `session/prompt` between turns — the pinned CLI's advertised slash command — fenced by the client's single-prompt guard; its `compaction_update`/`compaction_summary_chunk` notifications stay intentionally unprojected) plus the `UnavailableDevinRuntime`/`FakeDevin` seam. Service-test `fixture` calls adjusted for main's dropped `desktop` parameter. Reviewed release inventory recomputed to 235 entries; install pins refreshed for the merged `src/cli.ts`.
- 2026-09-17: converged-tree final gate evidence — `bun run check` green end to end: support-runtime, cost-surfaces (44), install-pins, effect-architecture, security-primitives (48 files), eslint, tsc; scripts 2305/2306 (one 5s browser-handoff timeout under load ~12, 20/20 in isolation), plugins 166+36, `bun test ./src --isolate --max-concurrency=1` 6217/6217 clean, convex 407, pr 22, site 188, app 668; build:site --check, build:app, build clean; `check:package` green in a governed-ref clone (history scan over governed refs, reviewed 235-entry inventory — digest recomputed after the Devin adapter edit changed a shipped file size, isolated local+global consumer verification). Delivered as PR #252 (`devin/session-compaction-main` @ ed8b427f → main); PR #246 closed as superseded release-line variant.
