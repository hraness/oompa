...
Cloud sync is optional. Local provider profiles, Codex credentials, Claude Code configuration and credentials, Devin configuration and credentials, and local execution continue to work without it. Oompa identity is separate from every provider account.
...
- Codex and Claude Code personal-session adoption status: whether discovery is enabled and bounded pending, adopted, and fenced counts. Candidate identities and records are never included. Devin has no personal-home adoption surface; Devin account identity and allowance are not projected, and Oompa reports only local signed-in readiness plus provider-supplied session context and cost facts in the neutral session stream.
