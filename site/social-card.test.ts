...
describe("social card", () => {
  ...
  test("paints the default dark theme with visible glyphs where the text sits", () => {
    ...
    // palette change. Pin this literal canonical release's geometry independently of
    // its colors, then compare the actual row against the same coverage.
    const mask = new Canvas(1200, 630, parseHexColor("#000000"));
    drawText(mask, socialCardFonts().book, "CLI v0.8.1 · canonical GitHub release · oompa.app", 88, 520, 30, parseHexColor("#ffffff"));
    ...
    expect(createHash("sha256").update(coverage).digest("hex"))
      .toBe("128a4ceaa96838da78367d9f5ac88ed77b5a2f823ace02c35fcb8c28ce77b130");
  ...
  test("keeps every card line inside its row and states the exact positioning text", () => {
    ...
    expect(lines.tagline).toBe("CLI v0.8.1 · canonical GitHub release · oompa.app");
