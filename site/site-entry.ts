...
const prRoot = document.querySelector<HTMLElement>("[data-pr-root]");
const prFilters = prRoot?.querySelector<HTMLElement>("[data-pr-filters]");
if (prRoot !== null && prFilters instanceof HTMLElement) {
  prFilters.hidden = false;
  prFilters.addEventListener("submit", (event) => event.preventDefault());
  const signals = [...prRoot.querySelectorAll<HTMLElement>("[data-pr-signal]")];
  const chips = [...prFilters.querySelectorAll<HTMLButtonElement>("[data-pr-filter]")];
  const search = prFilters.querySelector<HTMLInputElement>("[data-pr-search]");
  const status = prFilters.querySelector<HTMLElement>("[data-pr-status]");
  const empty = prRoot.querySelector<HTMLElement>("[data-pr-empty]");
  const groups = [...prRoot.querySelectorAll<HTMLElement>("[data-pr-severity-group]")];
  const apply = (): void => {
    const categories = new Set(chips.filter((chip) => chip.dataset.prFilter === "category" && chip.getAttribute("aria-pressed") === "true").map((chip) => chip.dataset.prValue ?? ""));
    const regions = new Set(chips.filter((chip) => chip.dataset.prFilter === "region" && chip.getAttribute("aria-pressed") === "true").map((chip) => chip.dataset.prValue ?? ""));
    const words = (search?.value.trim().toLocaleLowerCase() ?? "").split(/\s+/u).filter((word) => word.length > 0);
    let visible = 0;
    for (const signal of signals) {
      const show = (categories.size === 0 || categories.has(signal.dataset.prCategory ?? ""))
        && (regions.size === 0 || (signal.dataset.prRegions ?? "").split(" ").some((region) => regions.has(region)))
        && words.every((word) => (signal.dataset.prSearch ?? "").includes(word));
      signal.hidden = !show;
      if (show) visible += 1;
    }
    for (const group of groups) {
      group.hidden = group.querySelector("[data-pr-signal]:not([hidden])") === null;
    }
    if (status !== null) status.textContent = `${visible.toString()} of ${signals.length.toString()} signals`;
