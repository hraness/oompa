...
describe("canonical migration from authentic source-created schema49", () => {
  test("proves the additive canonical-key substep before joined62 preserves history and quarantines unproved runtime", async () => {
    ...
    expect(() => openStore(paths, true)).toThrow("STATE_SCHEMA_MIGRATION_REQUIRED:49:62");
    ...
    // claim that the complete joined62 upgrade adds only these four columns.
    ...
    expect(inspect(store, (database) => query(database, "PRAGMA user_version"))).toEqual([{ user_version: 62 }]);
